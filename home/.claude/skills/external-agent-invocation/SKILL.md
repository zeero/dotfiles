...
数分以上かかる見込みの呼び出しは、結果待ちに入る前に見張りをバックグラウンドで仕込む（例: `sleep 180` 後にログ行数と対象プロセスの CPU 時間を測って報告させる）。ハングは静かに起きるため、見張り無しでは発見が人の指摘まで遅れる（52分無進捗の実例）。検知したら原因究明より先に kill して同じ入力で再実行する — 一過性で、再実行が正常に完走する事例が多い。
