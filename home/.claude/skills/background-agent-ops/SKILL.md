...
- **対処**:
  ...
  - **報告の長さに上限を指示する**。teammate の完了報告は一定長で切り詰められ（`[result truncated — ask the agent for the rest via SendMessage]`）、続きを取るたびに往復が要る。調査依頼では「各項目は要点と根拠のファイル:行だけ」「全体で N 行以内」と上限を書くか、長い結果はファイルへ書かせてパスだけ返させる。
