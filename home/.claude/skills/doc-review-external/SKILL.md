...
- codex は trusted directory（リポジトリ内の cwd）から実行してください。scratchpad 等リポジトリ外の cwd では "Not inside a trusted directory" で即失敗します（exit 1。`; echo` 等のラッパーを挟むと exit 0 に見えるため、出力が数行ならこのエラーを疑う）。対象ファイル自体はリポジトリ外でも構いません。
