...
- [Quick Start](https://pi.dev/docs/latest/extensions#quick-start)
- [Extension Locations](https://pi.dev/docs/latest/extensions#extension-locations)
- [Available Imports](https://pi.dev/docs/latest/extensions#available-imports)
- [Writing an Extension](https://pi.dev/docs/latest/extensions#writing-an-extension)
  - [Extension Styles](https://pi.dev/docs/latest/extensions#extension-styles)
- [Events](https://pi.dev/docs/latest/extensions#events)
  - [Lifecycle Overview](https://pi.dev/docs/latest/extensions#lifecycle-overview)
  - [Resource Events](https://pi.dev/docs/latest/extensions#resource-events)
  - [Session Events](https://pi.dev/docs/latest/extensions#session-events)
  - [Agent Events](https://pi.dev/docs/latest/extensions#agent-events)
  - [Model Events](https://pi.dev/docs/latest/extensions#model-events)
  - [Tool Events](https://pi.dev/docs/latest/extensions#tool-events)
- [ExtensionContext](https://pi.dev/docs/latest/extensions#extensioncontext)
- [ExtensionCommandContext](https://pi.dev/docs/latest/extensions#extensioncommandcontext)
- [ExtensionAPI Methods](https://pi.dev/docs/latest/extensions#extensionapi-methods)
- [State Management](https://pi.dev/docs/latest/extensions#state-management)
- [Custom Tools](https://pi.dev/docs/latest/extensions#custom-tools)
- [Custom UI](https://pi.dev/docs/latest/extensions#custom-ui)
- [Error Handling](https://pi.dev/docs/latest/extensions#error-handling)
- [Mode Behavior](https://pi.dev/docs/latest/extensions#mode-behavior)
- [Examples Reference](https://pi.dev/docs/latest/extensions#examples-reference)
...
To share extensions via npm or git as pi packages, see [packages.md](https://pi.dev/docs/latest/packages).
...
See [Session Format](https://pi.dev/docs/latest/session-format) for session storage internals and the SessionManager API.
...
Fired on compaction. See [compaction.md](https://pi.dev/docs/latest/compaction) for details.
...
Fired on `/tree` navigation. See [Sessions](https://pi.dev/docs/latest/sessions) for tree navigation concepts.
...
Fired before each LLM call. Modify messages non-destructively. See [Session Format](https://pi.dev/docs/latest/session-format) for message types.
...
Transforms chain across handlers. See [input-transform.ts](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/examples/extensions/input-transform.ts).
...
UI methods for user interaction. See [Custom UI](https://pi.dev/docs/latest/extensions#custom-ui) for full details.

### ctx.hasUI

`false` in print mode (`-p`) and JSON mode. `true` in interactive and RPC mode. In RPC mode, dialog methods (`select`, `confirm`, `input`, `editor`) work via the extension UI sub-protocol, and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) emit requests to the client. Some TUI-specific methods are no-ops or return defaults (see [rpc.md](https://pi.dev/docs/latest/rpc#extension-ui-protocol)).
...
Read-only access to session state. See [Session Format](https://pi.dev/docs/latest/session-format) for the full SessionManager API and entry types.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `pi` / command `ctx`; see [Session replacement lifecycle and footguns](https://pi.dev/docs/latest/extensions#session-replacement-lifecycle-and-footguns).
...
Subscribe to events. See [Events](https://pi.dev/docs/latest/extensions#events) for event types and return values.

### pi.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](https://pi.dev/docs/latest/extensions#custom-tools) for full details.
...
**Important:**`promptGuidelines` bullets are appended flat to the `Guidelines` section with no tool name prefix. Each guideline must name the tool it refers to — avoid "Use this tool when..." because the LLM cannot tell which tool "this" means. Write "Use my\_tool when..." instead.
...
Register a custom TUI renderer for messages with your `customType`. See [Custom UI](https://pi.dev/docs/latest/extensions#custom-ui).

### pi.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](https://pi.dev/docs/latest/keybindings) for the shortcut format and built-in keybindings.
...
Set the current model. Returns `false` if no API key is available for the model. See [models.md](https://pi.dev/docs/latest/models) for configuring custom models.
...
See [custom-provider.md](https://pi.dev/docs/latest/custom-provider) for advanced topics: custom streaming APIs, OAuth details, model definition reference.
...
**Important:**`promptGuidelines` bullets are appended flat to the `Guidelines` section with no tool name prefix or grouping. Each guideline must name the tool it refers to — avoid "Use this tool when..." because the LLM cannot tell which tool "this" means. Write "Use my\_tool when..." instead.
...
See [examples/extensions/ssh.ts](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/examples/extensions/ssh.ts) for a complete SSH example with `--ssh` flag.
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](https://pi.dev/docs/latest/tui) for the full component API and [tool-execution.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
For the exhaustive list of keybinding ids and defaults, see [keybindings.md](https://pi.dev/docs/latest/keybindings). `keybindings.json` uses those same namespaced ids.
...
**For custom components, see [tui.md](https://pi.dev/docs/latest/tui)** which has copy-paste patterns for:
...
See [tui.md](https://pi.dev/docs/latest/tui) for the full component API.
...
See [tui.md](https://pi.dev/docs/latest/tui) for the full `OverlayOptions` API and [overlay-qa-tests.ts](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/examples/extensions/overlay-qa-tests.ts) for examples.
...
See [tui.md](https://pi.dev/docs/latest/tui) Pattern 7 for a complete example with mode indicator.
...
All render functions receive a `theme` object. See [themes.md](https://pi.dev/docs/latest/themes) for creating custom themes and the full color palette.
...
| JSON (`--mode json`) | No-op | Event stream to stdout, see [json.md](https://pi.dev/docs/latest/json) |
