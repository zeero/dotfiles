...
2. 既存メモリファイルをサンプリングして読む（ストア全体に散らして最大8件程度）。以下を推定する:
   ...
   - 本文の規約（例: `**Why:**` / `**How to apply:**` 構造、エントリ間の `[name](name.md)` 相互参照）
   - ファイル命名規約（kebab-case slug か `type_topic.md` か、など）
3. **発見したものを維持する。** ステージ済みストアへ書くすべてのファイルは発見済みフォーマットに一致させる。新しい frontmatter フィールドの導入、type のリネーム、本文スタイルの変更はしない。ただし `description` と index フックの**文言**は例外で、「recall 最適化（Sharpen）」操作で能動的に磨いてよい — 維持するのは構造（フォーマット）であって、文言（content）は最適化対象である。
...
### 重複の統合（Merge）
統合は最も安全で、最も効き目の大きい操作である。**「同じ事実の重複」を探すだけでは足りない** — 同じトピックについて別々のエントリに散った断片も統合対象とする。ストア全体をトピック単位で見渡し、1つの見出しの下にまとまるものを積極的に束ねる。

- 束ねるかどうかの基準は「そのエントリ群は、想起されるとき常に一緒に効くか」。yes なら1ファイルにまとめる。
- 統合後は最も明瞭な表現を残し、矛盾しない詳細は合併する。相互参照はリンク切れしないよう維持する。
- 見送ってよいのは、想起される状況が実際に異なるときだけ。「別々のファイルに書かれているから」は見送る理由にならない。
- 統合候補が1件も無かった場合は、その旨をレポートに明記する。無言だと点検した結果なのか見落としなのか区別がつかない。
...
### recall 最適化（Sharpen）
生き残ることが確定したエントリ（統合後・未変更を含む。要確認フラグ中のものは対象外）について、`description` とインデックス行のフックが「その事実が関係する状況」で確実に想起されるかを点検する。recall はホストが現在の状況と `description` の意味的重なりで判定するため、**結論ではなく想起されるべき状況を先頭に置く**（front-load）。過剰リライトは既存の良い表現を壊すため、保守的に行う:

- **状況の front-load**: 先頭が事実・結論から始まっているものは、その事実が関係する状況（どの操作・ファイル・ツールを触るときか）を先頭へ立て直す。
- **状況アンカーは温存（事実の主語は温存でない）**: 先頭が「recall 時に手元で触っている状況・ツール・ファイル」を名指しているもの（例:「scripts/symlink.sh を触るとき」）は、それ自体が最良の想起フックなので**触らない**。ただし先頭の固有名詞が〈想起される状況〉ではなく〈その事実の主語〉にすぎない場合（例:「X は Y である」の X）は温存対象ではない — 固有名詞が先頭にあるという理由だけで温存せず、状況を front-load し直す。
- **no-op の除去**: 想起を左右しない主観的な言い回しや冗長な背景説明は削る。
- **意味は保存**: 語順・語彙の最適化であって、事実そのものは書き換えない。事実が変わるものは統合/矛盾解消/削除の管轄。
- **フォーマットは不変**: 対象は `description` と index フックの**文言**のみ。frontmatter フィールド・type・本文スタイル・命名規約は「フォーマット発見」で維持したものを変えない。

これは既存エントリへの変更なので、リライトしたものはレポートの「recall 最適化」に before → after で記録し、承認ゲートで確認できるようにする。

### 本文の標準見出し・参照記法への正規化（Normalize）

生き残るエントリ（要確認フラグ中のものは対象外）の本文を、発見済み規約の標準形へ揃える。事実の追加・削除はせず、形式だけを直す:

- **標準見出しへの再編**: 本文が標準見出し（例: `**Why:**` / `**How to apply:**`）を欠き、別見出しや箇条書きで実質同じ内容を持つ場合、その内容を対応する標準見出しの下へ振り分け直す。見出しに対応する内容が本文に無いものは、無理に捻出せず欠落のままにする。
- **wikilink の置換**: 本文中の `[[...]]` 記法は使用禁止。メモリ間参照は `[name](name.md)` へ、メモリ外（skill・リポジトリ内ファイル等）への参照はバッククォートのパス表記へ置換する。wikilink はプレーン Markdown として解決されず、リンク先不在も検出できないためである。

正規化したものはレポートの「正規化」に対象ファイルと内容（見出し再編 / wikilink 置換）を列挙する。
...
- **変更サマリ**: 追加 / 統合 / 削除 / 要確認 / recall 最適化 / 正規化 の各件数
...
- **recall 最適化**: `<file>` ごとに before → after（想起性のための語順・語彙調整、事実は不変）
- **正規化**: `<file>` ごとに、何を正規化したか（見出し再編 / wikilink 置換）
