...
- **購読してから最低1分は unsubscribe できない**（公式 doc: "You can unsubscribe after subscribing after at least one minute."）。制約に反した解除要求は失敗コードを返すだけで例外にならないため、**戻り値を確認しないと購読が黙って残り、購読枠を消費し続ける**。短時間の試走を繰り返すスクリプトほど当たる。
- **購読枠は OpenD 全体で共有される**。`query_subscription(is_all_conn=True)` は他プロセス（常駐 daemon 等）が保持している購読まで返す。追加購読を伴うツールを作る前に残枠を実測する。
