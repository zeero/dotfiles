...
Obsidian Vault `/Users/zeero/Documents/memos` の `Wiki/` ディレクトリを LLM が管理する。
...
2. **対象ファイルのコミットハッシュ取得**:
   ```bash
   git -C /Users/zeero/Documents/memos log -1 --format="%H" -- <source_path>
   ```
...
2. **対象ファイル列挙**:
   ```bash
   cd /Users/zeero/Documents/memos
   git log --since="<期間>" --name-only --pretty=format: -- <raw_paths> | sort -u | grep -E '\.md$'
   ```
   - mtime ベースで列挙したい場合は `find <raw_paths> -mtime -1 -name '*.md'` を使う
3. **逐次 ingest**: 各ファイルについて単一 ingest 手順 2-9 を実行（再 ingest 判定があるので未更新は自動スキップ）
4. **サマリー報告**: 取り込んだ件数・スキップ件数・更新ページ数

`<raw_paths>` は schema の `raw_paths` を参照。
