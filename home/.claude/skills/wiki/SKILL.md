...
Obsidian Vault `~/Documents/memos` の `Wiki/` ディレクトリを LLM が管理する。
...
2. **対象ファイルのコミットハッシュ取得**:
   ```bash
   cd ~/Documents/memos && git log -1 --format="%H" -- <source_path>
   ```
3. **再 ingest 判定**: `Wiki/refs/{ディレクトリ}-{ファイル名}.md` を確認し、`source_commit` が一致すれば「既に最新です」と報告してスキップ
   ...
   current=$(cd ~/Documents/memos && git log -1 --format='%H' -- <source_path>)
...
2. **対象ファイル列挙**: 以下のいずれか（`<raw_paths>` は schema の `raw_paths` を参照）
   ...
   cd ~/Documents/memos && git log --since="<期間>" --name-only --pretty=format: \
...
3. **必要なら全文検索で補完**:
   ```bash
   rg -il "<キーワード>" ~/Documents/memos/Wiki/pages/
   ```
