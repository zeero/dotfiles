...
export default function (pi: ExtensionAPI) {
  pi.on("tool_result", async (event, ctx) => {
    ...
    if (!input.path || !input.path.endsWith(".sh")) return;

    // UIステータスを更新
    ctx.ui.setStatus("shellcheck", `Checking ${input.path}...`);

    try {
      // check-shell.sh を実行 (JSON入力を渡して ShellCheck & Git をチェック)
      ...
      const hookScript = process.env.HOME + "/.agents/hooks/check-shell.sh";
      const result = await pi.exec("sh", ["-c", `echo '${payload}' | bash "${hookScript}"`], { signal: ctx.signal });

      // ステータスをクリア
      ctx.ui.setStatus("shellcheck", "");
      ...
      try {
        ...
        statusMsg = json.systemMessage || "Shell checks completed.";
        ...
        if (result.code === 0) {
          statusMsg = "✅ Shell checks passed.";
          additionalText = `\n\nShell checks completed for ${input.path}.`;
          ctx.ui.notify(statusMsg, "info");
        } else {
          statusMsg = "❌ Shell checks failed!";
          const output = result.stdout || result.stderr || "No output from hook script.";
          additionalText = `\n\n[Shell Hooks] Found issues in ${input.path}:\n\n${output}`;
          ctx.ui.notify(statusMsg, "error");
        }
      }
      ...
      ctx.ui.setStatus("shellcheck", "");
      const errMsg = `ShellCheck hook error: ${error instanceof Error ? error.message : String(error)}`;
