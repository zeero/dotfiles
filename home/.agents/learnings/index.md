...
- worktree で external レビューを頼むときはコミットしてからコミットハッシュで連携する — worktree-external-review-commit-then-hash.md — external レビューアは stale 索引で現 HEAD とズレた前提に倒れる。commit+hash で git の事実に固定し「索引でなく git で確認」と指示する
